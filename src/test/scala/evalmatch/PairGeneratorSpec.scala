...
class PairGeneratorSpec extends Specification { 
  ...
  "A PairGenerator" should { 
    ...
    "create pairs from multiple applications" in { 
      ...
      val e2 = Employee("e2")
      val e3 = Employee("e3")
      val a1 = Application(e1, List(e3))
      val a2 = Application(e2, List(e3))

      val pairs = pairGenerator.createPairs(List(a1, a2))
      pairs must be equalTo List(EvaluationPair(e3, e1, 0.0, 0), EvaluationPair(e3, e2, 0.0, 1))
    }
    ...
    "ignore double pair defenitions" in { 
      ...
      val pairs = pairGenerator.createPairs(List(a1, a2))
      pairs must be equalTo List(EvaluationPair(e2, e1, 0.0, 0))
    }
  }
